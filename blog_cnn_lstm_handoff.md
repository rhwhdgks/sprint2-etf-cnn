...
결과 — **`ensemble_best` = `logistic_image_scale` + `cnn_1d_attention_image_scale` + `cnn_1d_cumulative_scale`** (rank 평균):

| 지표 | 값 | 비교 |
|---|---|---|
| OOS rank corr | **0.0422** | logistic_image 단일(0.039) 천장 돌파 |
| Top-k Sharpe | **0.503** | cnn_1d_cumulative 단일(0.52)에 근접 |
| 두 metric 동시 상위권 | ✓ | 이번 실험 유일 |
...
- CNN-only 앙상블의 한계 (0.039 천장)와 **logistic을 섞어 돌파**한 `ensemble_best` (rank 0.042 / Sharpe 0.503)
...
>
> **② 앙상블은 "같은 family 여럿"보다 "다른 family 혼합"이 더 효과적.** CNN끼리만 섞어서는 천장을 못 뚫었지만, logistic 하나만 태워도 rank corr 0.032 → 0.042로 뚫렸다. LSTM도 같은 논리로 합류시키는 것.

...
**하지만 앙상블도 `logistic_image_scale` 0.039는 못 넘는다.** CNN 가문 안에서 아무리 섞어도 이 천장이 있다.

---

## 6. 왜 앙상블이 천장에 막혔나 — CNN 모델들이 "너무 비슷"하다

모델 간 raw score 상관을 찍어보면:

![Model correlation](ode_inputs_cnn/figures/05_model_raw_correlation.png)

CNN 1D 계열끼리 **0.5~0.6 상관**. 비슷한 정보를 다른 방식으로 표현하고 있을 뿐이다. 앙상블의 힘은 "**상관 낮은 신호**를 합칠 때" 나오는데, 지금 구성원들은 이미 너무 닮았다.

CNN이 놓치는 축은 뭘까? 힌트가 하나 있다:
...
2. **상관이 낮으면 앙상블 폭발력이 커진다** — `logistic_image` 천장 0.039를 뚫을 현실적 카드
3. **발표 스토리가 완성된다**: "이미지(CNN = 공간) + 시퀀스(LSTM = 시간)의 **상보성(complementarity)**"이라는 해석
...
### ✅ 지금까지 얻은 것
- 7개 CNN 아키텍처의 OOS 성능 맵
- 이미지 변환과 CNN 기여를 분리한 ablation
- Rank corr와 portfolio Sharpe가 **다른 챔피언**을 가리키는 현상 발견
- 앙상블의 효과와 한계 (0.039 천장)
- ODE 스프린트가 바로 집어 쓸 수 있는 μ·Σ·R·risk 번들

### 🎯 다음에 할 것
- LSTM 합류: walk-forward 포맷 통일해서 같은 평가 그리드에 올리기
- CNN+LSTM 앙상블로 `logistic_image` 천장 넘기 시도
- 성공 시 ODE 스프린트에 `ensemble_cnn_lstm` 번들 전달

### 🧠 한 줄 교훈
> **"새 모델이 최고 성능을 낸다"는 이야기는 반드시 비CNN 베이스라인과 함께 비교해야 설득력이 있다.** 만약 logistic을 베이스로 두지 않았다면, 우리는 "CNN이 잘 된다"고 말하고 넘어갔을 것이다. 이미지 변환의 가치를 놓쳤을 것이고, LSTM을 얹을 근거도 약했을 것이다.

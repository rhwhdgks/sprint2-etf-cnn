...
ROOT = Path("ode_inputs_cnn/ensemble_top3")
...
| 지표 | 1위 모델 | 값 |
|---|---|---|
| OOS rank correlation | `logistic_image_scale` (non-CNN) | 0.0392 |
| Top-k portfolio Sharpe | `cnn_1d_cumulative_scale` | 0.521 |
| 앙상블 (top-3 CNN) | `ensemble_top3` | rank 0.032 / Sharpe 0.374 |

→ 이미지 변환 자체가 lift의 대부분을 만들고, CNN은 그 위에서 **포트폴리오 레벨 개선**에만 기여. 다음 단계로 시퀀스 모델(LSTM) 합류가 자연스러움 — [blog 글](blog_cnn_lstm_handoff.md) 참고.
